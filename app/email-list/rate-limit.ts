...
function getRuntimeCache() {
  if (!runtimeCache) {
    runtimeCache = getCache({ namespace: "email-list-form-rate-limit" });
  }
...
function getIpKey(ip: string) {
  ...
  if (!secret) {
    throw new Error("CONTACT_FORM_SECRET is required for email-list rate limiting.");
  }

  return createHmac("sha256", secret).update(ip).digest("hex");
}

export async function isEmailListSubmissionAllowed(ip?: string) {
  if (!ip) {
    return true;
  }
  ...
  try {
    ...
    if (!record || record.resetAt <= now) {
      await cache.set(
        ...
        { ttl: windowSeconds, name: "email-list-form-attempts" },
      );
      return true;
    }
    ...
    await cache.set(
      ...
      { ...record, count: record.count + 1 },
      {
        ...
        name: "email-list-form-attempts",
      },
    );

    return true;
  } catch (error) {
    console.error("Email-list form rate limit check failed open.", error);
    return true;
  }
}
